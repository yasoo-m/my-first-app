...
import { getStoreName, parseDate, parseNumber, taxExclude, removeHyphen, resolveProductCode, resolveCost, resolvePostalCode, resolvePaymentMethod, buildRow } from './helpers';
...
export async function convertAupay(rows: string[][], brand: BrandType): Promise<ConversionResult> {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    try {
      ...
      const productCode = await resolveProductCode(itemCode);
      ...
      const paymentMethod = await resolvePaymentMethod(settlementName);
